...
cl = LINETCR.LINE()
#cl.login(qr=True)
cl.login(token='EqcvWxSWkZLXvNuO60S3.FXP8F/ZJ0UQHfIxYmKf0iW.zVXWYhHzAUu3+pcJ0cImPjX9peYm+h/P5+1r29K3m5Y=')
cl.loginResult()
...
kk = LINETCR.LINE()
#kk.login(qr=True)
kk.login(token='EqVo3wChQiVBUcrkwJd6.Lkg8ohUsFOz1yZrKJtfpHG.bDITRQLGCRayVWCMAxyaG/1G5vmpmn5GoLoP/JjMp54=')
kk.loginResult()
print "Kk-Login Success\n"

kc = LINETCR.LINE()
#kc.login(qr=True)
kc.login(token='EqOkpru1zwRb2gyxJdy9.th5xFRYfLrNmTFAuPc9Aoq.4q8Y2ZxiceV843Eo+o5SRUDBwf+Rd9ROMC6v+c9djYM=')
kc.loginResult()
print "Kc-Login Success\n"

kr = LINETCR.LINE()
#kr.login(qr=True)
kr.login(token='EqpIEsc8V5CIkvbHYqk3.TAVOkm2wqPizxdXz1JiGmW.K40rS8y4sgAaTwmiAJrLn9C0r3pgPkU5aRSOyKEmVQU=')
kr.loginResult()
print "Kr-Login Success\n"

km = LINETCR.LINE()
#km.login(qr=True)
km.login(token='EqGxCp0X3le8o96beLH6.MC3I4QsKSrV9tmqyKt6jHG.Ntpi1bcHGQGv+E3QE0LrZIHvdSbJfFKhdrf6RPXx4aw=')
km.loginResult()
...
╚═════════════════════════
"""
tjia="u7f1a7c7fece3d2eaa11625f232417423"
    ...
    try:
                        ...
                        if op.param1 in cctv['point']:
                            ...
                            if Name in cctv['sidermem'][op.param1]:
                                ...
                                if " " in Name:
                                    nick = Name.split(' ')
                                    if len(nick) == 2:
                                        random.choice(KAC).sendText(op.param1, "ฮั่นแน่ " + "☞ " + nick[0] + " ☜" + "\nแอบอ่านอยู่ละสิ. . .\nออกมาคุยบ้างก็ได้นะ (-__-)   ")
                                    else:
                                        random.choice(KAC).sendText(op.param1, "ฮั่นแน่ " + "☞ " + nick[1] + " ☜" + "\nนี่ก็แอบอ่านอีกคน. . .\nออกมาคุยเดี๋ยวนี้รุย.... (-__-)   ")
                                else:
                                    random.choice(KAC).sendText(op.param1, "ต๊ะเอ๋ " + "☞ " + Name + " ☜" + "\nรู้นะว่ากำลังอ่าน???\nออกมาคุยเดี๋ยวนี้...(-..-)😁😁   ")
                        else:
                            pass
                    else:
                        pass
	    ...
	    if mid in op.param3:	        
		    ...
		    G = cl.getGroup(op.param1)
                    if len(G.members) <= wait["memberscancel"]:
                        cl.acceptGroupInvitation(op.param1)
                        cl.sendText(op.param1,"Maaf " + cl.getContact(op.param2).displayName + "\nสมาชิกในกลุ่มต้องมี 30 ขึ้น\nถึงจะเชิญผมเข้าได้, รึไม่ก็ติดต่อกับผู้สร้างผมได้ที่แชท!")
			...
			cl.updateGroup(G)
			cl.sendText(op.param1,"☆เรียกดูคำสั่ง ☞Help☜ เพื่อดูคำสั่งทั้งหมด☆\n☆^_^ ☆")
        ...
        if op.type == 17:
          if wait["Sambutan"] == True:
            ...
            cl.sendMessage(c)
            cl.sendText(op.param1,"สวัสดีคับ คนมาใหม่ " + cl.getContact(op.param2).displayName + "\nยินดีต้อนรับสู่ กลุ่ม ☞ " + str(ginfo.name) + " ☜" + "\nเข้ามาแล้วอย่าดูที่โน๊ตกลุ่มด้วยนะ\nอย่าลืมปิดเสียงการแจ้งเตือนด้วยล่ะ ^_^")
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["kickMention"] == True:
                     ...
                     balas = ["ขอโทษด้วยนะคับพีน้อง " + cName + "\nมันคือคำสั่งจากเบื้องบน! Sorry, Byee!!!"]
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     contact = cl.getContact(msg.from_)
                     cName = contact.displayName
                     balas = ["Sekali lagi nge tag gw sumpahin jomblo seumur hidup!","โปรดอย่าแทคนะคับ ขอร้อง",cName + " แทคทำไม?",cName + " แทคอีกแระ..กะบอกอยู่ว่าผมเป็นบอท??","แทคอีกทีเดะโดนดีด!","รึจะลอง", cName + " แทคทำไมคับ? มีอะไรรึป่าว?","ว่าไงท่าน\nไม่สนิทโปรดอย่าแทค " + cName, "จิแทคบอททำไมคับ " + cName, "พูดไม่รู้เรื่องหรือไงคับ " + cName + "?", "มีไรคับ " + cName + "?","มีไรคับ " + cName + " ตัวนั้นมันบอท!"]
                     ret_ = random.choice(balas)
                     name = re.findall(r'@(\w+)', msg.text)
                     mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                     mentionees = mention['MENTIONEES']
...
#		    cl.sendText(msg.to,"Khusus Nadya")
                ...
                if wait["Sambutan"] == True:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"เปิดระบบข้อความ คนเข้า คนออกヾ(*´∀｀*)ﾉ")
                else:
                    wait["Sambutan"] = True
                    wait["joinkick"] = False
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"เปิดอยู่แร้วววヽ(´▽｀)/")

            elif msg.text in ["Sambutan off"]:
                if wait["Sambutan"] == False:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"ปิดระบบข้อความ คนเข้า คนออก(　＾∇＾)")
                else:
                    wait["Sambutan"] = False
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"ข้อความปิดแล้ว(p′︵‵。)")
                ...
                wait["Sider"] = True
                cl.sendText(msg.to,"เปิดระบบจับตาดูคนอ่าน")
                
            elif "Sider off" in msg.text:
                if msg.to in cctv['point']:
                    cctv['cyduk'][msg.to]=False
                    wait["Sider"] = False
                    cl.sendText(msg.to, "ปิดระบบจับตาดูคนอ่าน")
                else:
                    cl.sendText(msg.to, "ได้คับท่าน ดีคับเจ้านาย")                         
		...
		if midd not in admin:
		    ...
		    cl.sendText(msg.to,"Admin Detected")
                ...
                midd = "u46a050ebcc66a90b47fae6256547cc53","u99cde2e2a4a4b11bfd4cc418913e8986","u1865fbab05ea885ca7bd481ec35c9a1d","u656b0ca994a1c9b462f9feb6f5ae3177","u46a050ebcc66a90b47fae6256547cc53"
                random.choice(KAC).inviteIntoGroup(msg.to,[midd])
		...
		if msg.from_ in Creator:
	        ...
	        if msg.from_ in Creator:                
              ...
              if msg.from_ in Creator:
            ...
            elif "Red copy @" in msg.text:
                   print "[COPY] Ok"
                   _name = msg.text.replace("Red copy @","")
                   _nametarget = _name.rstrip('  ')
                   gs = cl.getGroup(msg.to)
                   targets = []
                   for g in gs.members:
                ...
                try:
					...
					songname = msg.text.replace("ขอเพลง ","")
					params = {"songname": songname}
					r = requests.get('http://ide.fdlrcn.com/workspace/yumi-apis/joox?' + urllib.urlencode(params))
					data = r.text
					data = json.loads(data)
		...
		if msg.from_ in Creator:
                    string = msg.text.replace("/cnkapten","ஆัঐ௫နιшิa७ [BØT]")
		...
		if msg.from_ in Creator:
                    string = msg.text.replace("/cntc1","ஆัঐ௫နιшิa७ [BØT]")
                    if len(string.decode('utf-8')) <= 5000:
		...
		if msg.from_ in Creator:
                    string = msg.text.replace("/cntc2","ஆัঐ௫နιшิa७ [BØT]")
                    if len(string.decode('utf-8')) <= 5000:
		...
		if msg.from_ in Creator:
                    string = msg.text.replace("/cntc3","ஆัঐ௫နιшิa७ [BØT]")
                    if len(string.decode('utf-8')) <= 5000:
		...
		if msg.from_ in Creator:
                    string = msg.text.replace("/cntc4","ஆัঐ௫နιшิa७ [BØT]")
                    if len(string.decode('utf-8')) <= 5000:
                        profile = cl.getProfile()
                        profile.displayName = string
                        kr.updateProfile(profile)
