...
╠❂➣ [เปิดเม้น] เปิดคอมเม้น                                               ╠❂➣ [ปิดเม้น] ปิดคอมเม้น
...
╠══════════════════ 
╠❂➣ [Red on/off] 
╠❂➣ [เปิดลิ้ง]เปิดกันลิ้ง
╠❂➣ [ปิดลิ้ง]ปิดกันลิ้ง
╠❂➣ [Invite「mid」] เชิญด้วยmid
╠❂➣ [Kmid: ] สั่งเตะด้วยmid
╠❂➣ [Ginfo] สถานะห้อง
╠❂➣ [Cancel] ลบค้างเชิญ
...
╠❂➣ [หวัดดี @] หยอกเตะแล้วดึงกลับ
╠❂➣ [หวด @] เซลเตะ
╠❂➣ [Ban:]สั่งดำ mid
╠❂➣ [Unban:] ล้างดำ mid
╠❂➣ [เปิดกัน] เปิดป้องกัน
╠❂➣ [เปิดลิ้ง/ปิดลิ้ง] เปิด/ปิดกันลิ้ง
╠❂➣ [กันเชิญ] เปิดกันเชิญ
╠❂➣ [Cancel on] เปิดกันลบค้างเชิญ
╠══════════════════ 
║─┅═✥🌿คำสั่งสายขาว🌿✥═┅─  
╠══════════════════ 
╠❂➣ [Copy @] ก๊อปโปรไฟล์
...
╠═════════════════
║            ─┅═✥ᵀᴴᴬᴵᴸᴬᴺᴰ✥═┅─
║ •─✯͜͡ ✯RED★SAMURI★SELFBOT✯͜͡ ✯─•   「OR」\n╠-> help1\n╠-> help2\n╠-> help3\n╠-> help4\n╠-> help5
╠══════════════════
"""

helppro ="""
╠══════════════════ 
╠❂➣ [Read on/off][เปิด/ปิดระบบป้องกันทั้งหมด]
╠❂➣ [เปิดลิ้ง/ปิดลิ้ง]
╠❂➣ [กันเชิญ/ปิดกันเชิญ]
╠❂➣ [กันยก/ปิดกันยก]
╠❂➣ [เปิดกัน/ปิดกัน]
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     ...
                     balas = ["",cName + " มีไร ?, ", cName + " แทคทมาย? จะถามไรเชิญที่แชทส.ต,  " + cName + "?", " แทคอีกแระ? แทคแล้วไม่พูดโดนดีดนะ😄😄, " + cName + "?","ว่างายยย...มีอะไรกะว่ามา?, ", "จิแทคทำไมนักวุ้.. เดะปั๊ดจับปี้ซะรุย"]
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["kickMention"] == True:
                     contact = cl.getContact(msg.from_)
                     cName = contact.displayName
                     balas = ["",cName + " คุณทำผิดกฎ?, ", cName + " ไม่ต้องถามว่าโดนเพราะอะไร?,  " + cName + "?", "งงละดิ😆😆, " + cName + "?","เดะจิโดนไม่ใช่น้อย., ", "บอกแล้วไม่ฟัง -_-, "]
...
#========================== FOR COMMAND BOT STARTING =============================#
                ...
                try:
                    ...
                    if strnum == "off":
                        ...
                        if wait["lang"] == "JP":
                            cl.sendText(msg.to,strnum + "กลุ่มต่อไปที่ได้รับเชิญจะถูกปฏิเสธโดยอัตโนมัติ")
                        else:
...
#==============================================================================#
            ...
            elif msg.text in ["ปฏิทิน","Time","Waktu"]:
...
#================================  STARTED ==============================================#
		...
		for contact in group.members:
            ...
            elif "รันคท @" in msg.text:
                _name = msg.text.replace("รันคท @","")
                _nametarget = _name.rstrip(' ')
                gs = cl.getGroup(msg.to)
                for g in gs.members:
                    if _nametarget == g.displayName:
            ...
            elif "ดึงกลับ" in msg.text.split():
                if msg.toType == 2:
                    group = cl.getGroup(msg.to)
                    if group.invitee is not None:
                        try:
...
#------------------ KICK OUT FORM GROUP
        ...
        if op.type == 15:
          ...
          contact = cl.getContact(op.param2)
          image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
          random.choice(KAC).sendText(op.param1,"เฮ้ยๆ" + cl.getContact(op.param2).displayName + "จะรีบออกไปใหนแว้...😒😒")
          cl.sendImageWithURL(op.param1,image)
